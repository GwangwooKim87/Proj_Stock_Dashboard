...
## ✅ M6 재보강 8차: 총자산 시계열에 월간 추이 라인 추가 (2026-09-05)
- 기존 일별 라인(초록, 영역 채움) 위에 **월간 추이** 선(파랑 점선+원형 마커)을 추가 — 각 월의 마지막 스냅샷 값을 연결. `computeMonthlySeries(dates, values)` 신규 함수: 월별 마지막 인덱스 추출 → 월수익(전월 대비)·누적수익(시리즈 첫 값 대비) 계산.
- 월말 포인트에 마우스오버 시 기존 일 수익/누적 수익률 정보 아래 구분선과 함께 "[YYYY-MM 월말] 월수익 / 누적수익"을 금액+% 함께 표시(국내 주식 관례 색상).
- 차트에 두 시리즈를 구분하는 범례("총평가액"/"월간 추이") 추가.
- 검증: 1년치 랜덤 테스트 데이터(367일, 13개월)로 Node 재현 — 예: 2026-01-31 월말 월수익 +3.35%, 누적수익 +9.55% 정상 계산. `node --check` 통과, live 반영·마크업 확인.
