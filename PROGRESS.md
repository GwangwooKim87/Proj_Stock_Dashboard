...
## ✅ M6 재보강: 도넛 재렌더 버그, 원화 반올림, 모바일 아이콘, 티커 드래그, 손익 카드 (2026-09-05, 사용자 피드백)
- **핵심 버그 수정 — 종목별 비중 도넛이 최초 1회 후 사라짐**: `renderItemDonut()`에서 매 호출마다 `el.innerHTML=''`을 무조건 실행해 echarts가 관리하던 canvas DOM을 지워버림 → 필터 변경/새로고침 등 재렌더 시 차트가 깨짐. **최초 1회(인스턴스 미존재 시)에만** innerHTML을 비우도록 수정, 이후엔 `chart.resize()+setOption()`만 사용. "데이터 없음" 분기에서도 기존 인스턴스를 `dispose()` 후 정리하도록 보강.
- **원화 소수점 반올림**: `fmt()` 함수에서 `cur==='원'`이면 `Math.round()` 후 `maximumFractionDigits:0`로 표시 — KPI 카드 등에서 FX 환산 누적으로 생기던 "-3,695,050.29원" 같은 소수점 노출 제거.
- **모바일 새로고침/로그아웃 아이콘 깨짐**: 유니코드 문자(⟳/⏻)를 폰트 지원 여부에 따라 모바일에서 깨지거나 다르게 렌더링될 수 있어 인라인 SVG 아이콘(currentColor 상속)으로 교체.
- **증시 티커 스크롤바 숨김 + 드래그 스크롤**: `scrollbar-width:none`/`::-webkit-scrollbar{display:none}`로 스크롤바 시각적으로 제거, 마우스 mousedown/mousemove로 드래그 스크롤 구현(터치 스와이프는 기존 `overflow-x:auto`로 계속 동작).
- **총 누적 손익 카드 서브라인 보강**: 퍼센트 옆에 원화 손익액도 함께 표시 (`-5.35% · ▼ 3,695,050원` 형태), 총평가자산 카드와 표기 스타일 통일.
- 검증: Node로 `fmt()` 반올림·손익 서브텍스트 포맷 실데이터 재현, `node --check` 통과, live 반영 확인.

## ✅ M6 재보강 2차: 일수익 카드, 도넛 툴팁/멀티하이라이트/최초로딩 버그 (2026-09-05)
- **총 누적 손익 서브라인 원복 + "일 수익" 카드 신설**: 직전 세션에서 추가한 서브라인 원화 병기를 제거하고, 대신 전일 대비 변동(기존 `diffAmt`/`diffPct` 재사용)을 보여주는 **"일 수익" 카드**를 총평가자산-총누적손익 사이에 신설 (top row 2→3장).
- **핵심 버그 — PC 최초 접속 시 "표시할 종목이 없습니다"**: `updateItemDonutForFilter()` 호출이 `window._posItems` 대입보다 앞줄에 있어, 최초 로드 시 빈 배열로 도넛을 렌더링하던 순서 버그. 대입 이후로 호출 이동.
- **도넛 호버 시 텍스트 박스(툴팁) 제거**: 범례 볼드 강조만으로 충분하다는 피드백 → `tooltip:{show:false}`.
- **모바일 다중 하이라이트 방지**: 터치 이벤트에서 highlight가 downplay 없이 누적돼 여러 조각이 동시에 강조되던 문제 → mouseover/mouseout마다 `dispatchAction({type:'downplay'})`로 전체 해제 후 단일 강조.
- **등락률 부호/-0.00% 처리 통일**: `fmtPctSigned()` 신규 - 총평가자산·일수익·총누적손익 3곳의 %에서 발생 가능한 "-0.00%"(부동소수점 근사 0)를 "0.00%"로, 양수는 "+" 접두 일괄 적용.
- 검증: `node --check` 통과, 실데이터로 daily diff 재현(현재 시점 diff≈0, 정상), live 반영 확인.

## ✅ M6 재보강 3차: 보유 종목 테이블 모바일 카드 리스트 뷰 (2026-09-05)
- 모바일(폭 ≤640px)에서 8컬럼 테이블을 가로 스크롤로 보는 대신, **종목명(상단 고정) + 보유수량 + 평가금액 + 수익률** 4개 정보만 담은 카드 리스트로 전환. 미디어쿼리(`@media (max-width:640px)`)로 `.pos-wrap`(테이블)↔`.pos-cards`(카드) 표시 전환, 데스크톱은 기존 테이블 유지.
- `renderPosTable()`에서 테이블 렌더링과 동일한 필터링된 `items` 배열을 그대로 재사용해 카드 리스트도 함께 생성 → 계좌 필터([전체]/[키움]/[토스 국내]/[토스 해외])가 카드 뷰에도 동일하게 적용됨.
- 평가금액은 `qty*cur_price`로 별도 계산(테이블의 "현재가"와 구분되는 총평가액), 수익률 색상은 기존 테이블과 동일하게 초록(+)/빨강(-) 유지(다른 색상 규칙 혼재 방지).
- 검증: Node로 실데이터 5종목 렌더링 재현 확인, `node --check` 통과, live 반영·신규 마크업 확인.

## ✅ M6 재보강 4차: 필터 행에 평가금액/총손익 요약 표시 (2026-09-05)
- 보유 종목 필터([전체]/[키움]/[토스 국내]/[토스 해외]) 행 오른쪽에 현재 필터 기준 **평가금액**·**총손익**(초록/빨강)을 표시. 혼합 통화(예: [전체])는 KRW로 환산 합산.
- 데스크톱: 필터 버튼 그룹과 같은 행 우측에 인라인 표시. 모바일(폭≤640px)은 공간 부족으로 필터 버튼 바로 아래 별도 카드로 표시(`@media (max-width:640px)`로 세로 스택 전환).
- `renderPosTable()`에서 이미 필터링된 `items`를 그대로 재사용해 계산 - 테이블/카드 리스트와 항상 동일한 필터 상태 유지.
- 검증: `docker exec` 내부 Python으로 4개 필터 조합 실데이터 합산 검증(키움+토스국내+토스해외 = 전체 정합 확인). `node --check` 통과, live 반영 확인.

## ✅ M6 재보강 5차: 필터 요약 총손익 색상/퍼센트 (2026-09-05)
- 필터 행 요약의 "총손익" 색상을 초록/빨강 → **상승(+)빨강/하락(-)파랑**(국내 주식 관례, 다른 KPI 카드와 통일)으로 수정.
- 총손익 옆에 **손익률(%)** 추가 표시 (`fmtPctSigned` 재사용, 투자원금 KRW 환산 합산 기준). 예: "총손익 -1,523,146원 (-7.49%)".
- 검증: `docker exec` 내부 실데이터로 4개 필터 손익률 재현(전체 -5.35%, 키움 -7.49%, 토스국내 -21.28%, 토스해외 +0.70%) 확인. `node --check` 통과, live 반영 확인.

## ✅ M6 재보강 6차: 필터 요약 색상 CSS 명시도 버그, 일 수익 카드 삭제 (2026-09-05)
- **필터 요약 색상 미반영 버그**: `.filter-summary b{color:#e8e8e8}` 규칙(클래스+엘리먼트 결합, 명시도 0,1,1)이 `.red`/`.blue` 단일 클래스(0,1,0)보다 우선해 색상이 무시됨 — KPI 카드에서 겪은 것과 동일한 유형의 버그. `.filter-summary b.red`/`.filter-summary b.blue` 결합 선택자로 재정의.
- **"일 수익" 카드 삭제** (사용자 요청): 총평가자산-총누적손익 사이에 있던 일 수익 카드 제거, top row 3장→2장으로 원복.
- 검증: `node --check` 통과, live 반영·마크업 확인(일 수익 미노출, 필터 요약 색상 CSS 존재).

## ✅ M6 재보강 7차: 총자산 시계열 툴팁에 일 수익 추가 (2026-09-05)
- **배경**: 총자산 시계열 추이 차트는 일일 크론 스냅샷(하루 1건) 시계열인데, 마우스오버 시 표시되던 "변동률"이 사실 (총평가액-총투자원금)/총투자원금 기준 **누적 수익률**이라 하루 단위 변화를 보여주지 못한다는 지적. "일 수익" 개념을 이 툴팁에 넣는 방향으로 재검토.
- **수정**: 차트 포인트(i)와 바로 전 스냅샷(i-1)의 `total_values` 차이로 **일 수익**(전일 스냅샷 대비 금액+등락률, 국내 주식 관례 색상 상승 빨강/하락 파랑)을 계산해 툴팁에 추가. 기존 "변동률"은 "**누적 수익률**"로 라벨을 명확히 해 유지(둘 다 의미가 다르므로 함께 표시). 시계열 첫 포인트는 비교 대상이 없어 "데이터 없음 (첫 스냅샷)"으로 표시.
- 검증: 실데이터(2026-09-04→09-05, 총평가액 65,468,167→65,418,897원) 기준 Node로 재현 → "일 수익 ▼ 49,270원 (-0.08%)" 정상 계산 확인. `node --check` 통과, live 반영·마크업 확인.

## ✅ M6 재보강 8차: 총자산 시계열 "월간" 필터 탭 추가 (2026-09-05, 요구사항 정정)
- **1차 시도(오버레이 라인) 방향 오해로 되돌림**: 처음엔 일별 라인 위에 월말 포인트를 잇는 보조선을 얹는 방식으로 구현했으나, 사용자 의도는 "**7일/30일/월간/전체 필터 탭에 '월간'을 추가**해서 선택 시 전체 데이터를 월 단위로 집계해 보여주는 것"이었음. 오버레이 방식 코드 제거 후 재구현.
- **최종 구현**: 기간 필터에 `월간` 탭 신규 추가(7일/30일/월간/전체). "월간" 선택 시 `/api/portfolio/history?range=all`로 전체 일별 데이터를 받아 클라이언트에서 `computeMonthlyPoints()`로 월말(각 월의 마지막 스냅샷) 기준 집계 → 그 결과로 차트를 다시 그림(오버레이 아님, 탭 전환 시 완전히 새 차트).
- 월간 뷰의 각 포인트 호버 시: **총평가금액**, **월수익**(전월 대비 금액+%), **누적수익**(집계 시작 시점 대비 금액+%) 표시, 국내 주식 관례 색상(상승 빨강/하락 파랑).
- Y축 동적 도메인 계산(`calcYDomain`)과 억/만 단위 축 포맷(`yAxisFmt`)은 일별/월별 공용으로 분리.
- 검증: 1년치 랜덤 테스트 데이터(367일)로 Node 재현 → 13개월 정상 집계(예: 2026-01 월수익 +3.35%, 누적수익 +8.78%). `node --check` 통과, live 반영·마크업 확인.
- **참고**: 이번 요청과 무관하게, 테스트 목적으로 `day_snapshots`에 1년치(2025-09-04~2026-09-03) 랜덤워크 데이터 367건이 들어가 있음(코드/커밋 아님, DB만). 실제 운영 데이터가 아니므로 확인 끝나면 삭제 필요.
