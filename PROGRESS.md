...
## 📌 M5 진행 시 반영 예정 (2026-09-05 사용자 지시, 아직 미착수)
- **"브로커" → "증권사" 명칭 변경**: 테이블 헤더 등 전체 라벨 변경.
- **증권사명 한글 표기**: `kiwoom`→**키움증권**, `toss`→**토스증권** 매핑 (향후 증권사 추가 시 매핑 테이블 확장).

## ⏭ 다음 세션 (남은 작업)
  1. **UI/UX M5**: 보유 종목 테이블 - 정렬 규칙, 통화 포맷, 토스 원화 수익률 버그 수정, -0.00% 방지, 계좌 필터 탭, hover/칩 스타일 + 당일 등락률 컬럼 + 모바일 가로 스크롤 (신규 추가 항목)
  2. **AI 브리핑 카드** — 미진행 (유예)
  3. (선택)**네이버 뉴스 수집+변동성 필터링** — 미진행 (미룸)
